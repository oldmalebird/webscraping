...
xfd_grain_price = client.xfd_db.xfd_grain_price  # xfd_grain_price collection
...
def get_price(url):
    ...
    for tr in trs:
        ...
        if xfd_grain_price.find({
                'date': date,
                'veg_name': veg_name,
                'specification': specification
        }).count() > 0:
            print(
                xfd_grain_price.find({
                    'date': date,
                    'veg_name': veg_name,
                    'specification': specification
                }).count())
        ...
        if (',' not in low_price) and (',' not in avg_price) and (
            ...
            price_id = xfd_grain_price.insert(data)  #insert db
...
if __name__ == '__main__':
    ...
    urls = [
        'http://www.xinfadi.com.cn/marketanalysis/5/list/{}.shtml'.format(
