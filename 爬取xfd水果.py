...
xfd_fruit_price = client.xfd_db.xfd_fruit_price  # xfd_fruit_price collection
...
def get_price(url):
    ...
    for tr in trs:
        tds = tr.find_all('td')
        name = list(tds[0].stripped_strings)[0]
        ...
        if xfd_fruit_price.find({
            ...
            #dup_count += 1
            print(date, name, specification, unit, avg_price)
            print('已爬取到已保存过的数据')
            DUPLICATE_FLAG = True
            break
            # if dup_count > 2:
        ...
        if (',' not in low_price) and (',' not in avg_price) and (
            ...
            price_id = xfd_fruit_price.insert(data)  #insert db
